...
glucose_min <- 70  # minimum plausible fasting glucose level in mg/dL
glucose_max <- 200 # maximum plausible fasting glucose level in mg/dL
...
# Apply the caps to ensure values are within realistic human limits
glucose <- pmin(pmax(glucose_raw, glucose_min), glucose_max)
cholesterol <- pmin(pmax(cholesterol_raw, cholesterol_min), cholesterol_max)
BMI <- pmin(pmax(BMI_raw, BMI_min), BMI_max)
blood_pressure <- pmin(pmax(blood_pressure_raw, blood_pressure_min), blood_pressure_max)
