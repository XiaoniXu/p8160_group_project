...
# delete this, just keeping for myself/potential troubleshooting later

# # test
# testing <- lm_varyss_storebetas(fxn_gen = generate_t_copula_data, samp_sizes = vary_sampsize, tdist_degfree = tdist_df) # it's not storying the point estimate lol WHY
# 
# 
#   
# testblock <- lm(gendata_ss1_test$Glucose ~ gendata_ss1_test$Cholesterol + gendata_ss1_test$BMI + gendata_ss1_test$BloodPressure)
# # CIs
# testinggg <- as.data.frame(confint(testblock))
# colnames(testinggg) <- c("lower_95", "upper_95")
# testinggg <- testinggg[2:4,]
# rownames(testinggg) <- c("Gluc_Chol", "Gluc_BMI", "Gluc_BP")
# # point values
# testinggg$point_val <- testblock$coefficients[2:4]
# # that works, in this order!
# # and add marker
# testinggg$gensampsize <- 3
```

```{r}
# simstudy1_listdfs <- lm_varyss_storebetas(fxn_gen = generate_t_copula_data, samp_sizes = vary_sampsize, tdist_degfree = tdist_df)
...
num_simulations = 100 # WILL BE 1000 -- but start with 100 to show it works!
...
# code below is redundant, just need to plot!
