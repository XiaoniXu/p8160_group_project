...
comparison_matrix <- data.frame(
  ...
  Target_Correlation = c(0.5, 0.0, 0.0, 0.0, 0.0, 0.4),  # From defined Sigma
...
* *Interpretation: "vary sample size, correlation level, or degree of non-normality". In the code below, we choose from two of these options: to vary 1. sample size and 2. skewness.*
  * *Interpretation: "assess bias in parameter estimates, type I error in hypothesis testing, and coverage in CIs". In the code below, we a) assess bias in parameter estimates by estimating the coefficients under each setting with 1000 simulations, and b) calcualte the percentage of 95% CIs from those simulations [calculated under the incorrect assumption of normality] for each setting which include the true, theoretical value for a given regression coefficient (thereby giving us the empirical risk of a 'Type I error', i.e. the probability that the incorrectly generated 95% CI will exclude the true value of the regression coefficient [parameter]).*
...
Multiple linear regression assumes a linear relationship between Y and all X, normally distributed residuals, and no multicollinearity in the set of covariates. The last two of these assumptions are both violated by correlated data we have generated. With skewed, heavy-tailed t-distributions for the independent and dependent variables, the model residuals are likely to not be non-normally distributed; and, with the correlation structure we induced, BMI and BP (two of our covariates) are highly correlated. We expect to see biased estimates and inference in using this regression structure to model our data, which are highly unsuited to it.

First, we vary the generated sample size to examine the effect of sample size on bias & inference. Second, we vary the degrees of freedtom to the t-distribution to examine the effect of skewness (i.e. deviation from normality) on bias and inference.
...
- For each sample: Simulate 1000 times
...
### Interpretation:
*True regression coefficient values are plotted in green, above, for reference.* 
...
As sample sizes increased, the probability of the MLR 95% CIs *including* the true regression coefficient values decreased, indicating that the probability of those CIs *excluding* the true regression coefficient values increased. This trend was clear across all three covariates. The most likely explanation is that, as sample size increased, the MLR model became more confident in its estimates and produced narrower 95% CIs, thereby showing more clearly the bias in its estimates. This explains why the probability of the 95% CI excluding the true value was so much higher for Cholesterol; as we saw in the first plot, the MLR model produced much more biased estimates of this parameter than the other two, and so as the CIs shrank towards that biased value, the probability of a Type I error increased.
...
As the degrees of freedom to the t-distributions increased increased, the bias of the model estimated regression coefficients decreased. This is as we would expect. The t-distribution approaches the normal distribution as the degrees of freedom approaches infinity; therefore, as we increased the degrees of freedom, we lessened the violation of the MLR that we had introduced by using skewed, non-normal independent and dependent variable distributions. (Note: we see that precision was not affected by changes in skewness i.e. approximate normality, which is as expected, given that sample size was fixed in these correlations.)

As the degrees of freedom increased (i.e. less deviation from normality), the probability of the MLR 95% CIs *including* the true regression coefficient values increased, indicating that the probability of those CIs *excluding* the true regression coefficient values decreased This trend was clear across all three covariates. In fact, all three converged around 95% probability, indicating that as the t-distributions approached normality, the 95% CIs generated by MLR (under the assumption of normality) converged to being _true_ 95% CIs.
