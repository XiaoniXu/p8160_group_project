...
blood_pressure <- blood_pressure_raw[blood_pressure_raw >= blood_pressure_min & 
                                       blood_pressure_raw <= blood_pressure_max]
...
The estimated mean values of the generated dataset are very close to the intended means(100, 200, 25, 120) used in the data generation process. This suggests that the location parameter of the multivariate t-distribution is well preserved.  
...
# Fit (multivariable) linear regression -- assumes iid and errors/residuals ~ N(), 
# which can be violated with skewed/heavy-tailed distributions of explanatory or 
# outcome variables (which is the case for all variables we have generated)
...
# ```{r generate_data} -- due to the work in this chunk, the standard deviation was rescaled 
# (i.e. not dependent purely on t distribution df) -- numbers pulled out below:
...
tdist_df = 3 # arbitrary choice -- degree of heavy tails/variation from normality 
# (t dist df) fixed for these simulations

# custom fxn
# function: store beta & 95% CI (calculated assuming linearity) from each regression
# --varying samp size
lm_varyss_storebetas <- function(fxn_gen, samp_sizes, tdist_degfree){ # give a fxn to generate data, 
  # list of sample sizes to vary through, & degrees of freedom("df") of t dists (fixed in this simulation)
  ...
  for (nn in samp_sizes){
    ...
    # reformat to make it easy to combine/compress later for plotting 
    # -- each list entry is a dataframe
    # CIs
    ...
    # point values
    lm_outputs$point_val <- lm_nn$coefficients[2:4] # note: this works VERY specifically as 
    # written for our scenario--subsets are hard-coded for simplicity
...
```{r}
# Now: repeat 1000 times -- to get the % of CIs which cover the true value [in those 1000
# replications/simulations] [for a given sample size]
...
# get the percentage for which this was true among the [num_simulations] simulations 
# FOR each reg coeff (3) FOR each [sampsize] setting
simstudy1_1000sim_typeone <- simstudy1_1000sim |> 
  group_by(gensampsize, regcoeff) |> 
  summarise(in_CI_pct = mean(trueval_inCI))

# check
# simstudy1_1000sim[simstudy1_1000sim$regcoeff=="Gluc_BP" & simstudy1_1000sim$gensampsize == 147, ] 
# 3 false, matches up! : )
...
# custom fxn
# function: store beta & 95% CI (calculated assuming linearity) from each regression
# --varying df to t-dist (heaviness of tails)
lm_varyht_storebetas <- function(fxn_gen, samp_size, tdist_degfree){ # give a fxn to generate data, 
  # list of degrees of freedom to iterate through, & sample size for generated sample 
  # (fixed in this simulation)
  ...
  for (degfree in tdist_degfree){
    ...
    rownames(lm_outputs) <- c("Gluc_Chol", "Gluc_BMI", "Gluc_BP")
    # point values
    lm_outputs$point_val <- lm_nn$coefficients[2:4] # hard-coded for simplicity
...
```{r}
# Now: repeat this 1000 times -- to get the % of CIs which cover the true value 
# [in those 1000 replications/simulations] [for a given sample size]
...
run_simstudy1_varyht <- function(nsim){
  ...
  # unlist/combine, clean up -- no need to identify iterations
  simstudy1_nsim <- do.call(rbind, list_dfs) 
  rownames(simstudy1_nsim) <- seq(1,nrow(simstudy1_nsim))
  return(simstudy1_nsim)
}
```
...
The package used for MM-estimator robust regression was "robustbase" and the function used was "lmrob". Models were assessed by variance, bias, and R Squared value. Variance and R squared are reported in the output of lmrob, and bias was found by calculating the centered expected value of glucose using each model. Since the generated data was centered around zero, the expected value of glucose calculated from the model represents the actual expected value of glucose minus the true mean.
...
# Discussion & Synthesis of : Key Takeaways

### Data Generation
The application of a t-copula transform is crucial when dealing with heavy-tailed distributions; failure to appropriately model correlation between extreme values, particularly for heavy-tailed distributions such as the t-distribution, can lead to biased estimated and incorrect inference. In our data generating procedure, we induced a block-correlation structure using a t-copula transform with the specific goal of ensuring correlation between extreme values, which a Gaussian copula would have minimized. This method was successful, and our data-generation process produced empirical correlation matrices which agreed perfectly with our target correlation matrices. 

### Simulation Study 1

When data are not *iid* and normally distributed, fitting models which assume the data to be *iid* and ~ N(mu, sigma) will result in biased inference. Specifically, in Simulation Study 1, we showed that applying Multiple Linear Regression (MLR) to model t-distributed outcome and covariate variables results in biased inference and Type I error. With greater sample sizes the MLR converged towards biased estimates of the coefficients; the degree of bias increased with greater deviations from normality in the data (i.e. heavier t-distribution tails). The probability of a Type I error increased with a) larger sample sizes and b) greater deviations from normality (heavier t-distribution tails) in the data.

In the first part of the simulation, we varied sample size of the generated data. As sample size of our t-distributed data sets increased, the MLR model converged towards a set of biased parameter estimates for the regression coefficients. As sample sizes increased, the 95% CI grew narrower and narrower (due to the increased sample size). With the MLR converging towards biased estimates, this meant that the probability of a Type I error increased; the MLR 95% CIs were more likely to exclude the true value of the regression coefficients at larger sample sizes. This bias was most pronounced for the one covariate which was truly correlated with the outcome in the underlying data (i.e. Cholesterol--orrelated with the outcome, Glucose), and therefore this regression coefficient (for Cholesterol) also had the highest probability of a Type I error for inference. 

In the second part of the simulation, we varied the heaviness of the tails of the t-distributions generating our outcome and covariate data As we increased the degrees of freedom to the t-distribution, the MLR produced less biased outcomes, and the probability of the true value being included in the MLR 95% CIs approached 95%--that is, the 95% CIs generated by the model approached *true* 95% CIs. This is a clear demonstration of how deviations from normality in the data can lead to biased estimates and incorrect inference, as well as the converse: as our model approached normality (t-distibrution approaches normal distribution as df --> infinity), our estimates and inference with the MLR grew more and more valid. 

From Simulation Study 1, we conclude that anyone modeling data carefully examine the assumptions of their models. It is important to examine the distributions of your data *a priori* to inform your choice of model--and it is crucial to *not* choose a model when its fundamental assumptions are violated by shape and structure of multivariate data, even when that model is "simple" or "easier to implement". The assumptions of a model are not trivial: when data violate the key assumptions of a model (such as being correlated rather than *iid*, or being heavily-tailed rather than normally distributed), we get biased estimates and incorrect inference, as we saw in Simulation Study 1.

### Simulation Study 2

Simulation Study 2 allowed us to show how we can overcome the limitations of a misspecified model in Simulation Study 1: by choosing model formulations (in this case, regressions) which allow us to explicitly incorporate correlation and non-normality. In this simulation, MM robust regression allowed us to  capture the underlying structure of the data.

### Overall: 

Correct specification of a model is crucial to valid parameter estimates and statistical inference. In particular, choosing a model whose assumptions match the structure of the data is critical. When model assumptions are violated by the underlying correlation or distributional structures of the data, models which assume *iid* & normally distributed data produce invalid results. With knowledge or appropriate examination of the underlying correlation and distributional structures, well-suited models with a matching set of assumptions can be chosen to achieve unbiased estimates and valid inference
